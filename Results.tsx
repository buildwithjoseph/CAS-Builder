...
import { generateBlueprint, GHL_COMPARISON, MARKET_STACK_DATA } from '../constants';
...
export const Results: React.FC<ResultsProps> = ({ answers, onRestart }) => {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      // If content is longer than one page, simple approach: just fit width and let it scale down or crop (MVP)
      // For MVP we will just scale to fit width and split if needed, or simply render what fits. 
      // A robust multi-page split is complex. We'll stick to a long single page logic or fit-to-width.
      // Let's use auto-paging if possible, but jsPDF simple addImage is usually one page.
      // We will create a PDF that fits the height if it's reasonable, or just A4.
